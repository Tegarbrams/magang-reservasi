    ...
    <div class="flex h-screen overflow-hidden">
        ...
        <aside class="w-64 bg-gray-900 text-white flex-shrink-0">
            ...
            <nav class="p-4 space-y-2">
                ...
                <a href="{{ route('admin.fasilitas') }}" class="sidebar-active flex items-center gap-3 px-4 py-3 rounded-lg transition">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition">
                        <div class="p-6">
                            ...
                            <p class="text-sm text-gray-600 mb-4 line-clamp-2">{{ $item->deskripsi ?? 'Tidak ada deskripsi' }}</p>
                            <p class="text-2xl font-bold text-yellow-600 mb-4">Rp {{ number_format($item->harga, 0, ',', '.') }}</p>
                            <div class="flex gap-2">
                                <button onclick="openEditModal({{ $item->id }}, '{{ $item->nama }}', {{ $item->harga }}, '{{ addslashes($item->deskripsi ?? '') }}')" class="flex-1 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-lg transition">
                                    Edit
                                </button>
                                <button onclick="deleteFasilitas({{ $item->id }})" class="px-4 py-2 bg-red-600 hover:bg-red-700 text-white text-sm font-medium rounded-lg transition">
                                    Hapus
                                </button>
                            </div>
                        </div>
