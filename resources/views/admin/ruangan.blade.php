                        ...
                        <div
                            ...
                            <div class="p-6">
                                ...
                                <div class="space-y-2 mb-4">
                                    <p class="text-sm text-gray-600"><span class="font-semibold">Kapasitas:</span>
                                        {{ $ruangan->kapasitas }} orang</p>
                                    <p class="text-sm text-gray-600 line-clamp-2">
                                        {{ $ruangan->deskripsi ?? 'Tidak ada deskripsi' }}</p>
                                </div>
                                <p class="text-2xl font-bold text-yellow-600 mb-4">Rp
                                    {{ number_format($ruangan->harga, 0, ',', '.') }}</p>
                                <div class="flex gap-2">
                                    <button
                                        onclick="openEditModal({{ $ruangan->id }}, '{{ $ruangan->nama }}', {{ $ruangan->kapasitas }}, {{ $ruangan->harga }}, '{{ addslashes($ruangan->deskripsi ?? '') }}')"
                                        class="flex-1 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-lg transition">
                                        Edit
                                    </button>
                                    <button onclick="deleteRuangan({{ $ruangan->id }})"
