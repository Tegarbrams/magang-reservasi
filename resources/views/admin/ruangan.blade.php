...
<body class="bg-gray-50">
    ...
    <div class="flex h-screen overflow-hidden">
        ...
        <aside class="w-64 bg-gray-900 text-white flex-shrink-0">
            ...
            <nav class="p-4 space-y-2">
                ...
                <a href="{{ route('admin.ruangan') }}" class="sidebar-active flex items-center gap-3 px-4 py-3 rounded-lg transition">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <main class="flex-1 overflow-y-auto p-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition">
                        <div class="p-6">
                            ...
                            <div class="space-y-2 mb-4">
                                <p class="text-sm text-gray-600"><span class="font-semibold">Kapasitas:</span> {{ $ruangan->kapasitas }} orang</p>
                                <p class="text-sm text-gray-600 line-clamp-2">{{ $ruangan->deskripsi ?? 'Tidak ada deskripsi' }}</p>
                            </div>
                            <p class="text-2xl font-bold text-yellow-600 mb-4">Rp {{ number_format($ruangan->harga, 0, ',', '.') }}</p>
                            <div class="flex gap-2">
                                <button onclick="openEditModal({{ $ruangan->id }}, '{{ $ruangan->nama }}', {{ $ruangan->kapasitas }}, {{ $ruangan->harga }}, '{{ addslashes($ruangan->deskripsi ?? '') }}')" class="flex-1 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-lg transition">
                                    Edit
                                </button>
                                <button onclick="deleteRuangan({{ $ruangan->id }})" class="px-4 py-2 bg-red-600 hover:bg-red-700 text-white text-sm font-medium rounded-lg transition">
                                    Hapus
                                </button>
                            </div>
                        </div>
                    ...
                    <div class="col-span-3 text-center py-12 text-gray-500">
                        Belum ada ruangan. Klik tombol "Tambah Ruangan" untuk menambahkan.
                    </div>
                    @endforelse
                </div>
    ...
    <div id="formModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div class="bg-white rounded-xl max-w-md w-full mx-4">
            <div class="p-6 border-b border-gray-200 flex justify-between items-center">
                <h3 class="text-xl font-bold text-gray-800" id="modalTitle">Tambah Ruangan</h3>
            ...
            <form id="ruanganForm" method="POST" class="p-6 space-y-4">
                @csrf
                ...
                <div>
                    ...
                    <input type="number" name="kapasitas" id="kapasitas" required min="1" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-yellow-500 focus:border-transparent">
                </div>
    ...
    <script>
        function openAddModal() {
            ...
            document.getElementById('ruanganForm').action = '{{ route("admin.ruangan.store") }}';
        ...
        function openEditModal(id, nama, kapasitas, harga, deskripsi) {
            document.getElementById('modalTitle').textContent = 'Edit Ruangan';
            document.getElementById('ruanganForm').action = `/admin/ruangan/${id}`;
