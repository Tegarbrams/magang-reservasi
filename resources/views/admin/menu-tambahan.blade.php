...
<head>
    ...
    <title>Manajemen Menu Tambahan - Admin</title>
...
<body class="bg-gray-50">
    ...
    <div class="flex h-screen overflow-hidden">
        ...
        <aside class="w-64 bg-gray-900 text-white flex-shrink-0">
            ...
            <nav class="p-4 space-y-2">
                ...
                <a href="{{ route('admin.menu-tambahan') }}" class="sidebar-active flex items-center gap-3 px-4 py-3 rounded-lg transition">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"/>
                    </svg>
                    <span class="font-medium">Menu Tambahan</span>
                </a>
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
            ...
            <header class="bg-white border-b border-gray-200 px-8 py-4">
                <div class="flex items-center justify-between">
                    <div>
                        <h2 class="text-2xl font-bold text-gray-800">Manajemen Menu Tambahan</h2>
                        <p class="text-sm text-gray-600">Kelola data menu tambahan</p>
                    </div>
            ...
            <main class="flex-1 overflow-y-auto p-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    @forelse($menuTambahans as $menu)
                    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition">
                        <div class="p-6">
                            <div class="flex justify-between items-start mb-4">
                                <h3 class="text-xl font-bold text-gray-800">{{ $menu->nama }}</h3>
                            </div>
                            <p class="text-sm text-gray-600 mb-4 line-clamp-2">{{ $menu->deskripsi ?? 'Tidak ada deskripsi' }}</p>
                            <p class="text-2xl font-bold text-yellow-600 mb-4">Rp {{ number_format($menu->harga, 0, ',', '.') }}</p>
                            <div class="flex gap-2">
                                <button onclick="openEditModal({{ $menu->id }}, '{{ $menu->nama }}', {{ $menu->harga }}, '{{ addslashes($menu->deskripsi ?? '') }}')" class="flex-1 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-lg transition">
                                    Edit
                                </button>
                                <button onclick="deleteMenuTambahan({{ $menu->id }})" class="px-4 py-2 bg-red-600 hover:bg-red-700 text-white text-sm font-medium rounded-lg transition">
                                    Hapus
                                </button>
                            </div>
                        </div>
                    ...
                    <div class="col-span-3 text-center py-12 text-gray-500">
                        Belum ada menu tambahan. Klik tombol "Tambah Menu Tambahan" untuk menambahkan.
                    </div>
                    @endforelse
                </div>
    ...
    <div id="formModal" class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div class="bg-white rounded-xl max-w-md w-full mx-4">
            <div class="p-6 border-b border-gray-200 flex justify-between items-center">
                <h3 class="text-xl font-bold text-gray-800" id="modalTitle">Tambah Menu Tambahan</h3>
            ...
            <form id="menuTambahanForm" method="POST" class="p-6 space-y-4">
                @csrf
                ...
                <div>
                    ...
                    <input type="number" name="harga" id="harga" required min="0" step="0.01" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-yellow-500 focus:border-transparent">
                </div>
    ...
    <script>
        function openAddModal() {
            document.getElementById('modalTitle').textContent = 'Tambah Menu Tambahan';
            document.getElementById('menuTambahanForm').action = '{{ route("admin.menu-tambahan.store") }}';
        ...
        function openEditModal(id, nama, harga, deskripsi) {
            document.getElementById('modalTitle').textContent = 'Edit Menu Tambahan';
            document.getElementById('menuTambahanForm').action = `/admin/menu-tambahan/${id}`;
        ...
        function deleteMenuTambahan(id) {
            if (!confirm('Yakin ingin menghapus menu tambahan ini?')) return;

            fetch(`/admin/menu-tambahan/${id}`, {
                method: 'DELETE',
                headers: {
            ...
            .then(data => {
                if (data.success) {
                    alert('Menu tambahan berhasil dihapus');
                    location.reload();
                } else {
                    alert('Gagal menghapus menu tambahan');
                }
            });
        }
    </script>
